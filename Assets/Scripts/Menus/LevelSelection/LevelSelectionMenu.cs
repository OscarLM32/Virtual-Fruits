    ...
    public class LevelSelectionMenu : MonoBehaviour
    {
        ...
        private void SetUpLevels()
        {
            ...
            foreach (var level in levels)
            {
                ...
                if (IsLevelGreaterThanMax(level.levelId))
                    return;
            }
        }
        ...
        private bool IsLevelGreaterThanMax(string level)
        {
            ...
            if (int.Parse(levelValues[0]) > int.Parse(maxLevelReachedValues[0]))
                return true;

            if (int.Parse(levelValues[1]) > int.Parse(maxLevelReachedValues[1]))
                return true;
